...
void GetForwardDirection(float angle, float* dirX, float* dirY) {
    // Calculate forward direction based on angle
    if (angle == 0.0f) { // East
        *dirX = 1.0f;
        *dirY = 0.0f;
    } else if (angle == 90.0f) { // North
        *dirX = 0.0f;
        *dirY = -1.0f;
    } else if (angle == 180.0f) { // West
        *dirX = -1.0f;
        *dirY = 0.0f;
    } else if (angle == 270.0f) { // South
        *dirX = 0.0f;
        *dirY = 1.0f;
    }
}
...
int main(void) {
    ...
    while (!WindowShouldClose()) {
        ...
        if (IsKeyPressed(KEY_W)) { // Move forward
            GetForwardDirection(player.angle, &dirX, &dirY);
            float newX = player.pos.x + dirX * CELL_SIZE;
            float newY = player.pos.y + dirY * CELL_SIZE;
        ...
        if (IsKeyPressed(KEY_S)) { // Move backward
            GetForwardDirection(player.angle, &dirX, &dirY);
            float newX = player.pos.x - dirX * CELL_SIZE; // Opposite direction
            float newY = player.pos.y - dirY * CELL_SIZE;
            if (IsPointInMap(newX, newY) && map[(int)newY][(int)newX] != 'w') {
                player.pos.x = newX;
                player.pos.y = newY;
