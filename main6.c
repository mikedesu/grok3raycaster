...
#define MAP_HEIGHT 8
#define FOV 60.0f
#define CELL_SIZE 1.0f // Define cell size explicitly
#define PLAYER_OFFSET 0.5f // Half cell size for centering
...
int main(void) {
    ...
    while (!WindowShouldClose()) {
        ...
        if (IsKeyPressed(KEY_W)) { // Move forward
            float newX = player.pos.x + cosf(player.angle * DEG2RAD) * CELL_SIZE;
            float newY = player.pos.y + sinf(player.angle * DEG2RAD) * CELL_SIZE;
        ...
        for (int i = 0; i < numRays; i++) {
            ...
            // Start rays from center of cell
        ...
        if (showDebugMap) {
            ...
            // Draw player at center of cell in debug view
