...
void GetMovementDirections(float angle, float* forwardX, float* forwardY, float* backwardX, float* backwardY) {
    ...
    if (angle == 0.0f) { // East
        ...
        *forwardX = 0.0f;
        *forwardY = -1.0f; // Forward is up (decreasing Y)
        *backwardX = 0.0f;
        *backwardY = 1.0f; // Backward is down (increasing Y)
        ...
        *forwardX = 0.0f;
        *forwardY = 1.0f; // Forward is down (increasing Y)
        *backwardX = 0.0f;
        *backwardY = -1.0f; // Backward is up (decreasing Y)
    }
}
