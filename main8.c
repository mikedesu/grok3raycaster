...
int main(void) {
    ...
    while (!WindowShouldClose()) {
        ...
        if (IsKeyPressed(KEY_W)) { // Move forward
            ...
            if (player.angle == 0.0f) directionX = 1.0f; // East: +X
            if (player.angle == 90.0f) directionY = -1.0f; // North: -Y
            if (player.angle == 180.0f) directionX = -1.0f; // West: -X
            if (player.angle == 270.0f) directionY = 1.0f; // South: +Y
            float newX = player.pos.x + directionX * CELL_SIZE;
            float newY = player.pos.y + directionY * CELL_SIZE;
