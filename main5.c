...
typedef struct {
    ...
    float speed; // Not used for movement anymore, kept for potential future use
} Player;
...
int main(void) {
    ...
    Player player = {
        .pos = {1.0f, 1.0f}, // Start at exact grid position
        .angle = 0.0f, // Start facing east
        .speed = 5.0f // Unused now, but kept in struct
    };

    bool showDebugMap = true;

    while (!WindowShouldClose()) {
        float deltaTime = GetFrameTime(); // Still useful for other timing if needed

        if (IsKeyPressed(KEY_M)) showDebugMap = !showDebugMap;

        // Grid-based movement (1.0 unit steps)
        if (IsKeyPressed(KEY_W)) { // Move forward
            float newX = player.pos.x + cosf(player.angle * DEG2RAD) * 1.0f;
            float newY = player.pos.y + sinf(player.angle * DEG2RAD) * 1.0f;
